...
`cd customerSystem;` working directory

Build: `./mvnw compile -f "pom.xml";`

Run: `java -jar customerSystem-0.0.1-SNAPSHOT.jar;`

- Building compiles a fat jar to `target/customerSystem-0.0.1-SNAPSHOT.jar`
...
```bash
curl -X POST -H "Content-Type:application/json" -d '[JSON]' http://localhost:8080/customers
```
...
```bash
curl -X PUT -H "Content-Type:application/json" -d '[JSON]' http://localhost:8080/customers/[id]
```
where `id` is the id of the customer

where `JSON` is a JSON stringified customer object(Optionally with nested contact), to be added to the system
